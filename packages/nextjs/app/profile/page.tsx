...
import { Customer, Profile, Vendor } from "~~/types/utils";
...
export default function ProfilePage() {
  ...
  useEffect(() => {
    const parseResult = async () => {
      try {
        if (cusVen) {
          const dat = cusVen as unknown as Vendor | Customer;
  ...
  return (
    <div className="flex flex-col space-y-4 relative">
      {loading && <Loading />}
      <BackButton />
      <h1 className="text-bold text-xl">Profile</h1>
      <div className="flex flex-col space-y-4 justify-center">
        <div>
          <Image src={result.picture ?? ""} alt={"Image"} className="rounded-full w-40 h-40 m-2" />
        </div>
            ...
            (task.vendor === address || task.customer === address) && (
              <div key={task.id} className="card bg-base-100 max-w-96 space-y-3 shadow-md">
                ...
                <div className="card-actions justify-center">
                  ...
                  {isCustomer && (
                    <button onClick={() => Pay(task.price, task.name)} className="btn btn-success dark:btn-secondary">
                      Pay
                    </button>
                  )}
                </div>
